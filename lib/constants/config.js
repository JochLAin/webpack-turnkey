...
if (process.env.WEBPACK_TURNKEY_ESLINT) {
    const hasConfig = !exists('.eslintrc.js') && !exists('.eslintrc.json') && !exists('.eslintrc.yml') && !exists('.eslintrc.yaml');
    config.rules.push({
        enforce: "pre",
        test: /\.m?jsx?$/, 
        ...
        use: [{ 
            ...
            options: {
                fix: true,
                configFile: hasConfig ? undefined :
