...
let config = {
    rules: [
        ...
        { test: /\.(js|jsx)?$/, use: { loader: 'babel-loader', options: {
            presets: ['latest', 'env', 'stage-0', 'stage-1', 'stage-2', 'stage-3', 'react']
        }}},
        { test: /\.css$/, use: [{ loader: 'style-loader', options: { sourceMap: !!process.env.WEBPACK_TURNKEY_SOURCEMAP }}, { loader: 'css-loader', options: { sourceMap: !!process.env.WEBPACK_TURNKEY_SOURCEMAP }}] },
        { test: /\.s[ac]ss$/, use: [
